...
namespace skl{
	...
	template<class _KeyT, POOL_TYPE poolType = NonPagedPool>
	class set{
		//typedef typename skl::pair<_KeyT, _ValueT> item_type;
		typedef typename set<_KeyT> _MyT;
		
		class set_ref_item : public ref_item{
			friend class set;
			friend class ref_holder<set_ref_item>;
		public:
			set_ref_item(){}
			set_ref_item(const _KeyT& k) : first(k), ref_item(1){
			}
			_KeyT first;
		};
...
// 			return ptr;
	...
	public:
		set() { //:itr_end(*this, (PVOID)-1)
			RtlInitializeGenericTableAvl(&_root, &set::Compare, &set::Allocate, &set::Free, NULL);
		}
		~set() {
			clear();
		}
		...
		skl::pair<ref_holder<set_ref_item>, bool> find(const _KeyT& val) {
			set_ref_item temp_item;
			temp_item.first = val;
			if (!check_valid(temp_item.first))
				return skl::pair<ref_holder<set_ref_item>, bool>(ref_holder<set_ref_item>(nullptr), false);
			item_type temp;
			temp._item = &temp_item;
			auto result = (item_type*)RtlLookupElementGenericTableAvl(&_root, (PVOID)(&temp));
			return skl::pair<ref_holder<set_ref_item>, bool>(ref_holder<set_ref_item>(result ? result->_item : nullptr), false);
		}

		skl::pair<ref_holder<set_ref_item>, bool> remove(const _KeyT& val) {
			set_ref_item temp_item;
			temp_item.first = val;
			if (!check_valid(temp_item.first))
				return skl::pair<ref_holder<set_ref_item>, bool>(ref_holder<set_ref_item>(nullptr), false);
			...
			auto result = (item_type*)RtlLookupElementGenericTableAvl(&_root, (PVOID)(&temp));
			BOOLEAN ok = FALSE;
			ref_holder<set_ref_item> holder(result ? result->_item : nullptr);
			if (result){
			...
			return skl::pair<ref_holder<set_ref_item>, bool>(skl::move(holder), ok == TRUE);;
		}
		//T& operator[]( const Key& key );
		skl::pair<ref_holder<set_ref_item>, bool> insert(const _KeyT& key) {
			set_ref_item temp_item;
			temp_item.first = key;
			if (!check_valid(temp_item.first))
				return skl::pair<ref_holder<set_ref_item>, bool>(ref_holder<set_ref_item>(nullptr), false);
			...
			if (result && newElement) {
				result->_item = new(poolType)set_ref_item(key); //replace the stack memory
...
// 				}
			...
			return skl::pair<ref_holder<set_ref_item>, bool>(ref_holder<set_ref_item>(result ? result->_item : nullptr), newElement == TRUE);
		}
		...
		public:
			...
			iterator& operator++(){
				...
				value = ref_holder<set_ref_item>(rt ? rt->_item : nullptr);
				if (!rt){
					handle = (PVOID)-1;
				}
				return *this;
			...
			ref_holder<set_ref_item>& operator *() {
				return value;
			}
			set_ref_item* operator ->() {
				return value.get();
			}
