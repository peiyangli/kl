...
public:
	...
	bool init(int argc, const CharT* const argv[], const boost::program_options::options_description& opt);
	...
	boost::program_options::variables_map getContent()const{ return _cml; }
};

template<typename CharT>
bool ConfigCml::init(int argc, const CharT* const argv[], const boost::program_options::options_description& opt)
{
	bool ok = false;

	try{
		boost::program_options::variables_map cml;
		boost::program_options::store(boost::program_options::basic_command_line_parser<CharT>(argc, argv).options(opt).allow_unregistered().run(), cml);
		boost::program_options::notify(cml);
		_cml.swap(cml);
		ok = true;
	}
