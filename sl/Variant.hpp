...
public:
	...
	Variant() :m_typeIndex(typeid(void))/*, m_index(-1)*/
	{
	}
	...
	class = typename std::enable_if<Contains<typename std::remove_reference<T>::type, Types...>::value>::type>
		Variant(T&& value) : m_typeIndex(typeid(T))
	{
			//Helper_t::Destroy(m_typeIndex, &m_data);
	...
	template<typename T>
	typename const std::decay<T>::type& To(const T& def, bool* ok = nullptr)const
	{
		using U = typename std::decay<T>::type;
		if (!Is<U>())
		{
